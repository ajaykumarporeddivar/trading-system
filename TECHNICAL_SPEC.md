...
├── requirements.txt            # Package definitions (pandas, ccxt, sklearn, flask, etc)
├── run_247.py                  # Primary entrypoint, fault-tolerant continuous loop
...
├── agents/                     # Data/IO interfaces
│   ├── data_agent.py           # Async CCXT data puller + ta indicators
...
│   ├── training_export.py      # CSV performance export
...
├── web/                        # Flask Dashboard
│   ├── app.py                  # 8 API routes + V11 system status
│   ├── templates/
│   │   └── dashboard.html      # Multi-panel dashboard with Chart.js
│   └── static/
│       ├── css/dashboard.css   # Dark trading theme
│       └── js/dashboard.js     # Live data fetching, auto-refresh
...
- AI / MTF: `scikit-learn>=1.3.0`, `joblib>=1.3.0`
- Web / Automation: `flask>=3.0.0`, `APScheduler>=3.10.0`, `pytest-asyncio`
...
2. **Initialization**: It establishes the 6 `BaseAgent` implementations and boots the `RetrainScheduler` thread (`interval=3600`, `min_samples=500`).
3. **Execution**: Fires `asyncio.run(runner._run_all_timeframes())`.
